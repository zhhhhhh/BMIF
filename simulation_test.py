...
def bootstrap(address, seeds):
    ...
    #url = "http://" + address + "/bootstrap"
    #getURL = url + "?" + urllib.parse.urlencode(data)
    #req = urllib2.Request(getURL)
    
    req = urllib2.Request("http://" + address + "/bootstrap",
                          json.dumps(data).encode(encoding='UTF8'),
...
def simulate_tx(address, sender, receiver, amount):
    ...
    req = urllib2.Request(url="http://" + address + "/transactions/new",
                          headers={"Content-Type": "application/json"}, data=json.dumps(data).encode(encoding='UTF8'))
    res_data = urllib2.urlopen(req)
    res = res_data.read()
    return res
